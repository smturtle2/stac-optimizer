...
고정 batch schedule, 깊은 residual 모델, epoch별 validation loss curve,
...
`8.133 MB`로 줄였고, full-decay variant는 같은 메모리에서 trunk decay 규칙만
비교하며, wider cap은 더 많은 AdamW state를 써서 회귀 성능을 끌어올립니다.
이 수치는 저장소 내부 측정값이지 보편적 보장은 아닙니다.
